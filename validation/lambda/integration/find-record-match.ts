 ...
 * 
 * - The `token` field should uniquely identify the voter record and is used for caching purposes and 
 * further communication between the E-Signatures API and the external voter record. 
 ...
 * 
 * - The `methods` field should map valid signature verification methods (as defined by the `VerificationMethods` enumeration)
 * to boolean values, indicating which of them (if any) can be used to validate a signature submitted by
 * the voter. If no method can be used to validate the signature, the `NO_AVAILABLE_METHODS` value can be
 * used for this field.
 *
